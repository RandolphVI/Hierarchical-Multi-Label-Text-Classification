...
class BestCheckpointSaver(object):
    ...
    """
    def __init__(self, save_dir, num_to_keep=1, maximize=True):
    ...
    def handle(self, value, model, optimizer, global_epoch):
        """
        ...
        Args:
            ...
            global_epoch: The global epoch
        """
        state = {'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict()}

        current_ckpt = 'epoch-{0}'.format(global_epoch)
        filename = os.path.abspath(os.path.join(self._save_dir, current_ckpt))
        value = float(value)
        ...
        if self._maximize:
            should_save = not all(current_best >= value for current_best in best_checkpoints.values())
        else:
            should_save = not all(current_best <= value for current_best in best_checkpoints.values())
        if should_save:
            best_checkpoint_list = self._sort(best_checkpoints)

            worst_checkpoint = os.path.join(self._save_dir, best_checkpoint_list.pop(-1)[0])
